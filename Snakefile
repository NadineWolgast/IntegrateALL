...
rule all:
    input:
        ...
        #expand("STAR_output/{sample_id}/TSO_Line1.bam.bai", sample_id=list(samples.keys())),
        ...
        #expand("karyotype_prediction/{sample_id}.csv",sample_id=samples.keys()),
        expand("Sonja_Files/{sample_id}_output_report.csv",sample_id=list(samples.keys())),
    ...
    run:
        ...
        with open(output[0], "w") as output_file:
            if errors:
                ...
                # Extrahiere left und right Dateien aus der samples_csv
                samples = parse_samples(input.samples_csv)
                error_found = False  # Fehlerindikator initialisieren
                for sample_id, (left_file, right_file) in samples.items():
                    if left_file == right_file:
                        output_file.write(f"Error: Left and right files for sample {sample_id} are the same\n")
                        error_found = True  # Setze den Fehlerindikator
                if not error_found:
                    output_file.write("Sample format and file existence checks passed")


def parse_samples(samples_csv):
    samples = {}
    with open(samples_csv) as file:
        next(file)
        for line in file:
            sample_id, left, right = line.strip().split(",")
            samples[sample_id] = (left, right)
...
rule interactive_report:
    input:
        ...
        reads_per_gene="STAR_output/{sample}/ReadsPerGene.out.tab"
    ...
    shell:
        ...
        cp {input.reads_per_gene} interactive_output/{wildcards.sample}/{wildcards.sample}_$(basename {input.reads_per_gene}) &&
...
rule sonjas_rule:
    input:
        ...
        star_log_final_out_file="STAR_output/{sample}/Log.final.out",
        rna_seq_cnv_manual_an_table_file="RNAseqCNV_output/gatk/{sample}_gatk/manual_an_table.tsv",
        fastq1= lambda wildcards: samples[wildcards.sample][0]

    output:
        csv="Sonja_Files/{sample}_output_report.csv"

    shell:
        """
        python scripts/make_sonjas_report.py {wildcards.sample} {input.allcatchr_file} {input.aggregated_output} {input.karyotype} {input.fusioncatcher_file} {input.star_log_final_out_file} {input.rna_seq_cnv_manual_an_table_file} {output.csv}  {input.fastq1}
        """
...
rule final_classification:
    ...
    shell:
        """
        python scripts/make_final_classification_new.py {wildcards.sample} {input.allcatchr_file} {input.karyotype} {input.fusioncatcher_file} {input.arriba_file} {input.hotspots} {input.classification_file} {output.csv} {output.text} {output.curation} {output.driver}
        """
